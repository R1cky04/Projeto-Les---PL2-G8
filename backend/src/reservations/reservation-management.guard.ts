...
@Injectable()
export class ReservationManagementGuard implements CanActivate {
  canActivate(context: ExecutionContext): boolean {
    ...
    if (!actorRole || !MANAGE_ROLES.has(actorRole)) {
      throw new ForbiddenException({
        message: 'Apenas perfis IT, ADMIN, STAFF e FLEET podem gerir reservas.',
        code: 'RESERVATION_MANAGEMENT_ROLE_REQUIRED',
      });
    }
