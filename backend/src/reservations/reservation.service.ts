...
export class ReservationService {
  ...
  async create(
    ...
    if (expectedReturnAt.getTime() <= pickupAt.getTime()) {
      throw new BadRequestException(
        'A data de fim tem de ser posterior a data de inicio.',
      );
    }
  ...
  async update(
    ...
    if (current.status === 'CANCELLED' || current.status === 'COMPLETED') {
      throw new BadRequestException(
        'A reserva selecionada ja nao pode ser alterada.',
      );
    }

    let returnStation: StationRecord | null = null;

    if (payload.returnStationId) {
      returnStation = await this.stationService.findOne(
  ...
  async cancel(
    ...
    if (current.status === 'COMPLETED') {
      throw new BadRequestException(
        'A reserva ja foi concluida e nao pode ser cancelada.',
      );
    }
